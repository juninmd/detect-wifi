...
/**
 * Repository for managing detection history and tracked BSSIDs. Extracted from PreferencesUtil to
 * separate concerns.
 */
class DetectionHistoryRepository(context: Context) {
  ...
  companion object {
    ...
    @Volatile private var trackedBssidsCache: MutableSet<String>? = null
  ...
  private fun getOrLoadTrackedBssids(): MutableSet<String> {
    ...
    if (cache == null) {
      synchronized(cacheLock) {
        ...
        if (cache == null) {
          val loaded =
            getStringSet(PreferencesUtil.Companion.Keys.ALL_BSSIDS, mutableSetOf())
              ?: mutableSetOf()
