...
class SecurityPreferences(context: Context) : BasePreferences(context, PREF_NAME) {
  ...
  fun setSecurityAlertEnabled(enabled: Boolean) = putBoolean(KEY_SECURITY_ALERT_ENABLED, enabled)

  fun isSecurityAlertEnabled() = getBoolean(KEY_SECURITY_ALERT_ENABLED, false)

  fun setSecuritySoundEnabled(enabled: Boolean) = putBoolean(KEY_SECURITY_SOUND_ENABLED, enabled)
