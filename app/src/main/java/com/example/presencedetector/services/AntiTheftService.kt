...
class AntiTheftService :
  ...
  private val batteryLevelReceiver =
    object : BroadcastReceiver() {
      override fun onReceive(context: Context?, intent: Intent?) {
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    when (intent?.action) {
      ...
      ACTION_PANIC -> {
        val reason =
          intent.getStringExtra("com.example.presencedetector.EXTRA_REASON") ?: "PÂNICO MANUAL"
        triggerAlarm(reason)
      }
    }
    return START_STICKY
  ...
  private fun createForegroundNotification(): Notification {
    ...
    val pendingDisarmIntent =
      PendingIntent.getActivity(this, 0, disarmIntent, PendingIntent.FLAG_IMMUTABLE)
    ...
    val pendingAppIntent =
      PendingIntent.getActivity(this, 0, appIntent, PendingIntent.FLAG_IMMUTABLE)
  ...
  private fun handlePocketMode(event: SensorEvent) {
    ...
    // 2. Device WAS in pocket (or we assume it was if grace period passed? No, better to track
    // state)
  ...
  private fun triggerAlarm(reason: String) {
    ...
    try {
      val alarmIntent =
        Intent(this, com.example.presencedetector.security.ui.AlarmActivity::class.java).apply {
  ...
  private fun startRecording() {
    ...
    try {
      val fileName =
        "ALARM_" + SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date()) + ".m4a"
  ...
  private fun showAlarmNotification(reason: String) {
    ...
    val notification =
      NotificationCompat.Builder(this, NotificationUtil.SECURITY_CHANNEL_ID)
  ...
  override fun onTaskRemoved(rootIntent: Intent?) {
    ...
    if (isArmed) {
      ...
      val restartServiceIntent =
        Intent(applicationContext, AntiTheftService::class.java).apply { action = ACTION_START }
