...
/**
 * Background service for continuous presence detection. Runs even when the app is not in the
 * foreground. Ensures service keeps running via START_STICKY even when device is locked.
 */
class DetectionBackgroundService : Service() {
  ...
  private val appMonitorRunnable =
    object : Runnable {
      override fun run() {
        appUsageMonitor?.checkForegroundApp { packageName -> triggerHiddenCamera(packageName) }
  ...
  private fun checkForAutoArmSuggestion() {
    ...
    if (
      System.currentTimeMillis() - lastAutoArmSuggestionTime > 30 * 60 * 1000L
    ) { // Suggest every 30m if still unarmed

      Log.i(TAG, "Outside Safe Zone and unarmed. Suggesting Anti-Theft.")

      val enableIntent =
        Intent(this, com.example.presencedetector.receivers.NotificationActionReceiver::class.java)
          .apply { action = "com.example.presencedetector.ACTION_ENABLE_ANTITHEFT" }
      val pendingEnableIntent =
  ...
  private fun registerCameraReceiver() {
    cameraPresenceReceiver =
      object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
          if (intent?.action == CameraMonitoringService.ACTION_CAMERA_PRESENCE) {
            val cameraName =
              intent.getStringExtra(CameraMonitoringService.EXTRA_CAMERA_NAME) ?: "Unknown Camera"
      ...
      try {
        ...
        val notification =
          NotificationUtil.createForegroundNotification(
            this,
            getString(R.string.notif_monitoring_active),
            getString(R.string.notif_scanning_desc),
