...
class AlarmActivity : AppCompatActivity() {
        ...
        try {
            ...
            manager.openCamera(cameraId, object : CameraDevice.StateCallback() {
                override fun onOpened(camera: CameraDevice) {
                    ...
                    try {
                        camera.createCaptureSession(listOf(textureSurface, captureSurface), object : CameraCaptureSession.StateCallback() {
                                ...
                                try {
                                    ...
                                    Handler(Looper.getMainLooper()).postDelayed({
                                        try {
                                            ...
                                            // Orientation adjustment logic is complex, skipping for MVP "Surprise"
                                            // Front camera usually needs mirroring or 270 deg rotation.
                                            session.capture(captureReq.build(), null, null)
                                        } catch (e: Exception) { e.printStackTrace() }
                                    }, 1000) // 1s delay to ensure light/focus (even if blind)
    ...
    private fun saveAndSendPhoto(bytes: ByteArray) {
        try {
            val filename = "THIEF_TRAP_" + SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date()) + ".jpg"
