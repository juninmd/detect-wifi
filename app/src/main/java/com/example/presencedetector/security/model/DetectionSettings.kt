...
data class DetectionSettings(
    ...
    /** 
     * Período de graça em segundos para tolerar glitches no stream.
     * Se a detecção falhar por menos que esse tempo e voltar, 
     * o timer NÃO é resetado.
     */
    val gracePeriodSeconds: Int = 1,
    
    /** 
     * Cooldown em segundos entre notificações para a mesma câmera.
     * Evita spam de notificações se a pessoa ficar no local.
     */
    ...
    companion object {
        ...
        fun load(context: Context): DetectionSettings {
            ...
            val channels: List<CameraChannel> = try {
    ...
    fun addChannel(name: String, channelNumber: Int): DetectionSettings {
        ...
        return copy(
            ...
            enabledChannelIds = enabledChannelIds + newId
        )
    }
