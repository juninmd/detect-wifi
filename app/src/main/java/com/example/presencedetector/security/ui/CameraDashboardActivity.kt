...
class CameraDashboardActivity : AppCompatActivity() {
  ...
  private fun setupListeners() {
    ...
    adapter.setOnItemClickListener { channel ->
      val intent =
        Intent(this, CameraStreamActivity::class.java).apply {
  ...
  private fun toggleCameraVisibility(channel: CameraChannel) {
    // Atualiza o canal
    val updatedChannels =
      settings.channels.map { if (it.id == channel.id) it.copy(isHidden = !it.isHidden) else it }
  ...
  inner class CameraAdapter : RecyclerView.Adapter<CameraAdapter.CameraViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CameraViewHolder {
      val view =
        LayoutInflater.from(parent.context).inflate(R.layout.item_camera_dashboard, parent, false)
    ...
    inner class CameraViewHolder(itemView: View) :
      RecyclerView.ViewHolder(itemView), org.videolan.libvlc.interfaces.IVLCVout.Callback {
      ...
      fun bind(channel: CameraChannel) {
        ...
        itemView.setOnClickListener { onItemClick?.invoke(channel) }

        btnToggleVisibility.setOnClickListener { toggleCameraVisibility(channel) }
      }
      ...
      private fun startPreview(channel: CameraChannel) {
        ...
        val media =
          org.videolan.libvlc.Media(vlc, android.net.Uri.parse(url)).apply {
