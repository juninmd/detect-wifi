...
class SecuritySettingsActivity : AppCompatActivity() {
  ...
  private val notificationPermissionLauncher =
    registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
      if (isGranted) {
        ...
        Toast.makeText(this, "Permissão de notificação necessária para alertas", Toast.LENGTH_LONG)
          .show()
  ...
  private fun setupListeners() {
    ...
    binding.btnAutoDiscover.setOnClickListener { showAutoDiscoverDialog() }
      ...
      when {
        ...
        shouldShowRequestPermissionRationale(Manifest.permission.POST_NOTIFICATIONS) -> {
          AlertDialog.Builder(this)
            ...
            .setMessage(
              "O app precisa de permissão para enviar notificações de alerta quando uma pessoa for detectada."
            )
            .setPositiveButton("Permitir") { _, _ ->
              notificationPermissionLauncher.launch(Manifest.permission.POST_NOTIFICATIONS)
            }
            .setNegativeButton("Cancelar") { _, _ -> binding.switchMonitoring.isChecked = false }
            .show()
        }
        else -> {
          notificationPermissionLauncher.launch(Manifest.permission.POST_NOTIFICATIONS)
  ...
  private fun updateMonitoringState(enabled: Boolean) {
    ...
    if (enabled) {
      if (settings.channels.isEmpty()) {
        Toast.makeText(this, "Adicione pelo menos um canal antes de ativar", Toast.LENGTH_LONG)
          .show()
        binding.switchMonitoring.isChecked = false
        return
      }
  ...
  private fun showAddChannelDialog() {
    val dialogBinding =
      android.widget.EditText(this).apply { hint = "Nome da câmera (ex: Garagem)" }
  ...
  private fun updateChannelsList() {
    val channelNames =
      settings.channels.joinToString("\n") { channel ->
        "• ${channel.name} (Canal ${channel.channel})"
      }

    binding.tvChannelsList.text =
