...
class BluetoothDetectionServiceTest {
  ...
  fun `ScanCallback handles results`() {
    ...
    // We need to mock the scanner because Robolectric's ShadowBluetoothLeScanner might be tricky to
    // access directly via adapter in some versions,
    // but let's try assuming standard Shadow behavior.
    // Actually, we can just use reflection to access the private ScanCallback inside the service if
    // needed,
    ...
    service.setPresenceListener { _, devices, _ -> devicesFound = devices }
