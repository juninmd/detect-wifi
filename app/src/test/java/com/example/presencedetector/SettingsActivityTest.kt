...
class SettingsActivityTest {
  ...
  fun `switches update preferences`() {
    ...
    switchNotify.isChecked =
      true // Triggers listener? MaterialSwitch listener behavior in Robolectric depends, usually
    // needs manual trigger or performClick
  ...
  fun `sensitivity slider updates preference`() {
    ...
    slider.value = 2.0f
    // Listeners on Material Slider might not trigger just by setting value programmatically in
    // Robolectric without proper Shadow.
