...
class ClassifyDeviceUseCase {
  NetworkDevice execute(NetworkDevice device) {
    if (device.vendor != null) {
      ...
      if (vendor.contains('apple') || vendor.contains('samsung') || vendor.contains('motorola')) {
        return device.copyWith(category: DeviceCategory.smartphone);
      } else if (vendor.contains('intel') || vendor.contains('dell') || vendor.contains('hp')) {
        return device.copyWith(category: DeviceCategory.computer);
      } else if (vendor.contains('espressif') || vendor.contains('tuya') || vendor.contains('philips')) {
